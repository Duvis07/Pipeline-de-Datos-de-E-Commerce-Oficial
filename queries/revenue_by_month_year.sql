...
SELECT
    ...
    SUM(CASE WHEN strftime('%Y', s.order_delivered_customer_date) = '2016' THEN s.payment_value ELSE 0.0 END) AS Year2016,
    SUM(CASE WHEN strftime('%Y', s.order_delivered_customer_date) = '2017' THEN s.payment_value ELSE 0.0 END) AS Year2017,
    SUM(CASE WHEN strftime('%Y', s.order_delivered_customer_date) = '2018' THEN s.payment_value ELSE 0.0 END) AS Year2018
FROM stage s
